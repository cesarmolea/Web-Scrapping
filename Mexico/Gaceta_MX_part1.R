...
for (i in 1:length(df)){
  df$year <- as.factor(substr(rownames(df[i]), 42, 45))
  df$month <- as.factor(substr(rownames(df[i]), 55, 56))
  }
...
df$word_OCDE <- ifelse(str_detect(df$text, "ocde"), 1, 0)

# We remove stopwords in spanish
...
df_bm <- filter(df_bm, !grepl("banco|mundial", word))
df_bm

set.seed(1234)
wordcloud(words = df_bm$word, freq = df_bm$n, min.freq = 1,
          max.words = 25, random.order = FALSE, rot.per = 0.35, 
          colors = brewer.pal(8, "Dark2"))
...
df_fmi <- filter(df_fmi, !grepl("fondo|monetario|internacional|fmi", word))
df_fmi

set.seed(1234)
wordcloud(words = df_fmi$word, freq = df_fmi$n, min.freq = 1,
          max.words = 25, random.order = FALSE, rot.per = 0.35, 
          colors = brewer.pal(8, "Dark2"))

## Plotting the most common words per organization
par(mfrow = c(3,1))
barplot(df_bid[1:10,]$n, names.arg = df_bid[1:10,]$word,
        col ="lightblue", main ="Palabras más comunes BID",
        ylab = "Frecuencia de palabras")
barplot(df_bm[1:10,]$n, names.arg = df_bm[1:10,]$word,
        col ="red", main ="Palabras más comunes Banco Mundial",
        ylab = "Frecuencia de palabras")
barplot(df_fmi[1:10,]$n, names.arg = df_fmi[1:10,]$word,
        col ="green", main ="Palabras más comunes FMI",
        ylab = "Frecuencia de palabras")
